...
int main(int argc, char** argv) {
    ...
    const char *help[3] = {
        "Usage: addScalar -input inputfile -index numbertoadd -output outputfile\n"
        "Options:\n"
    ...
    if(!filter) filter = itk::AddScalar<unsigned char, 3>::New(3, inputtype);
    if(!filter) filter = itk::AddScalar<short, 3>::New(3, inputtype);
    if(!filter) filter = itk::AddScalar<unsigned short, 3>::New(3, inputtype);
    if(!filter) filter = itk::AddScalar<int, 3>::New(3, inputtype);
