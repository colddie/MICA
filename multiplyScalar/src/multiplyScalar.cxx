...
int main(int argc, char** argv) {
    ...
    const char *help[3] = {
        "Usage: multiplyScalar -input inputfile -index numbertomultiply -output outputfile\n"
        "Options:\n"
    ...
    if(!filter) filter = itk::MultiplyScalar<short, 3>::New(3, inputtype);
    if(!filter) filter = itk::MultiplyScalar<unsigned short, 3>::New(3, inputtype);
    if(!filter) filter = itk::MultiplyScalar<int, 3>::New(3, inputtype);
